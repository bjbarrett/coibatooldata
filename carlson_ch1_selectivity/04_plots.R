...
PlotToolPost <- function( model , almendra=0 , herm_crab=0 , color="black" , graph_title="words"){
        post <- extract.samples(model)
        guitar <- with(post , a + ba*almendra + bhc*herm_crab)
        dens(exp(guitar) , xlim=c(0 , 3100) , col=color , xlab="mass (g)")
...
PlotToolPost(model= mj2 , almendra=1 , color=palette[1] , graph_title= "Terminalia catappa")
PlotToolPost(model= mj2 , herm_crab=1 , color=palette[2] , graph_title= "Hermit crab")

#mc2
#coiba model astrocaryum and hermit crab
PlotToolPost <- function( model , astro=0 , river_snail=0 , color="black" , graph_title="words"){
        post <- extract.samples(model)
        nebula <- with(post , a + bas*astro + brs*river_snail)
        dens(exp(nebula) , xlim=c(0 , 3100) , col=color , xlab="mass (g)")
...
PlotToolPost(model= mc2 , astro=1 , color=palette[3] , graph_title= "Astrocaryum spp")
PlotToolPost(model= mc2 , river_snail=1 , color=palette[4] , graph_title= "Freshwater snail")
